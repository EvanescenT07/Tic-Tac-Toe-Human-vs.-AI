...
class TicTacToeGUI:
    ...
    def check_winner(self):
        for i in range(3):
            if self.board[3 * i] == self.board[3 * i + 1] == self.board[3 * i + 2] != ' ':
                self.update_scores(self.board[3 * i])
                return True
            if self.board[i] == self.board[i + 3] == self.board[i + 6] != ' ':
                self.update_scores(self.board[i])
                return True

        if self.board[0] == self.board[4] == self.board[8] != ' ':
            self.update_scores(self.board[0])
            return True

        if self.board[2] == self.board[4] == self.board[6] != ' ':
            self.update_scores(self.board[2])
            return True
    ...
    def end_game(self):
        if self.check_winner():
            winner = 'AI' if self.current_player == self.ai_player else 'Human'
